...
{ 
    if(isset($_POST['update']))
    {
        ...
        $sql="update  tblbooks set BookName=:bookname,CatId=:category,AuthorName=:author,ISBNNumber=:isbn where id=:bookid";
...
<head>
    ...
    <title>PESU MCA Library Management System | Edit Book</title>
...
<body>
    ...
    <div class="col-md-12">
    <h4 class="header-line" style="text-align: center;">Edit Book</h4>
    </div>
    </div>
    <div class="row">
    ...
    <form role="form" method="post">
        <?php 
        $bookid=intval($_GET['bookid']);
        $sql = "SELECT tblbooks.BookName,tblcategory.CategoryName,tblcategory.id as cid,tblbooks.AuthorName,tblbooks.ISBNNumber,tblbooks.id as bookid from  tblbooks join tblcategory on tblcategory.id=tblbooks.CatId where tblbooks.id=:bookid";
        $query = $dbh -> prepare($sql);
        $query->bindParam(':bookid',$bookid,PDO::PARAM_STR);
        $query->execute();
        ...
        <a href="manage-books.php" class="btn btn-danger btn-dm" role="button" aria-pressed="true">Cancel</a>
    </form>
    </div>
    </div>
    </div>
